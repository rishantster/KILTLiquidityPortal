...
**Program APR Calculation Enhancement**: Implementing pool-wide APR calculation based on ALL liquidity providers in the KILT/ETH Uniswap V3 pool, not just program participants. The system must calculate realistic Program APR using: (Daily Budget × 365) ÷ (Total Pool TVL from ALL LPs) × 100. This provides accurate APR that reflects the real competitive landscape where all KILT/ETH LPs are potential program participants. Average position size must be calculated from actual pool liquidity distribution, not just current program participants. This approach gives realistic expectations for what LPs can earn if they join the incentive program.
