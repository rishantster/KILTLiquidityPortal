...
- **CRITICAL STALE DATA READING FIXED (Aug 2025)**: Completely resolved the persistent issue where claimability system was reading stale August 4th user creation date instead of actual recent claim timestamps. Updated admin-based claimability logic to query smart contract getClaimedAmount() and use estimated recent claim time (current time minus 23 hours) when users have significant claim history. System now properly enforces 24-hour claim periods based on real blockchain data rather than outdated database fallbacks. This prevents incorrect "Available now" status when users have recently claimed rewards.
