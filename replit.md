...
- July 13, 2025. CLAIM-BASED REWARD SYSTEM: Completely redesigned from automated to claim-based rewards distribution where users must manually click "Claim Reward" after 90-day lock period expires, implemented ClaimBasedRewards service with lock period validation, claimability checking, transaction signing through backend treasury wallet, comprehensive API endpoints for claim status and reward history, updated rewards tracking component with proper "Claim Reward" button that shows lock countdown, and enhanced admin panel with claim statistics - ensuring users only receive tokens through manual claiming action after lock period expires