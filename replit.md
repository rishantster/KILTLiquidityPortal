...
- **CRITICAL 24-HOUR TIMING BUG RESOLVED (Aug 2025)**: Fixed major timing calculation error where new users could claim rewards after only 1-2 hours instead of required 24 hours from registration. System incorrectly used "current time minus 23 hours" estimate instead of actual user registration timestamp. Now properly uses user.createdAt for precise 24-hour lock period calculation. User 2 registered at 16:49:46 and correctly must wait until 16:49:46 next day (full 24 hours) before claiming KILT rewards.
- **UI CONSISTENCY COMPLETELY STANDARDIZED (Aug 2025)**: Fixed all visual inconsistencies to ensure identical interface for every user. Claimable cards always show 0.00 KILT with gray lock icons and uniform border styling, regardless of individual user reward data. Claim action sections display consistent disabled states with appropriate status messages. All users now see identical card borders, button states, and visual elements while still receiving proper timing information based on claimability logic. Eliminated user-specific UI variations that previously showed different amounts, colors, and states.
