...
export async function registerRoutes(app: Express, security: any): Promise<Server> {
  ...
  // === UNISWAP V3 LIQUIDITY MANAGEMENT ENDPOINTS ===
  ...
  app.post("/api/liquidity/add", async (req, res) => {
    try {
      ...
      if (!params.token0 || !params.token1 || !params.fee) {
        return res.status(400).json({ error: "Missing required parameters" });
      }

      res.json({ 
        message: "Add liquidity endpoint ready - requires frontend wallet integration",
        params: params,
        instruction: "Use frontend wallet connection to execute transaction"
      });
    } catch (error) {
  ...
  app.post("/api/liquidity/increase", async (req, res) => {
    try {
      const params = req.body;
      ...
      res.json({ 
        message: "Increase liquidity endpoint ready - requires frontend wallet integration",
        params: params,
        instruction: "Use frontend wallet connection to execute transaction"
      });
    } catch (error) {
  ...
  app.post("/api/liquidity/decrease", async (req, res) => {
    try {
      const params = req.body;
      
      if (!params.tokenId || !params.liquidity) {
        return res.status(400).json({ error: "Missing tokenId or liquidity parameter" });
      }

      res.json({ 
        message: "Decrease liquidity endpoint ready - requires frontend wallet integration",
        params: params,
        instruction: "Use frontend wallet connection to execute transaction"
      });
    } catch (error) {
  ...
  app.post("/api/liquidity/collect-fees", async (req, res) => {
    try {
      const { tokenId, recipient } = req.body;
      
      if (!tokenId || !recipient) {
        return res.status(400).json({ error: "Missing tokenId or recipient" });
      }

      res.json({ 
        message: "Collect fees endpoint ready - requires frontend wallet integration",
        tokenId,
        recipient,
        instruction: "Connect wallet on frontend to collect fees",
        note: "This will recover uncollected fees from closed positions"
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to collect fees" });
    }
  ...
  app.post("/api/liquidity/burn", async (req, res) => {
    try {
      ...
      res.json({ 
        message: "Burn position endpoint ready - requires frontend wallet integration",
        tokenId,
        instruction: "Connect wallet on frontend to burn position"
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to burn position" });
    }
  ...
  app.get("/api/liquidity/position/:tokenId/owner/:userAddress", async (req, res) => {
    try {
      const { uniswapV3Manager } = await import('./uniswap-v3-manager');
      const { tokenId, userAddress } = req.params;
      
      const isOwner = await uniswapV3Manager.isPositionOwner(tokenId, userAddress);
      
      res.json({ 
        tokenId,
        userAddress,
        isOwner,
        message: isOwner ? "User owns this position" : "User does not own this position"
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to check position ownership" });
    }
  });

  // Get detailed position information including uncollected fees
  app.get("/api/liquidity/position/:tokenId/details", async (req, res) => {
    try {
      const { uniswapV3Manager } = await import('./uniswap-v3-manager');
      const { tokenId } = req.params;
      
      const position = await uniswapV3Manager.getPosition(tokenId);
      
      res.json({
        tokenId,
        position,
        hasUncollectedFees: BigInt(position.tokensOwed0) > 0 || BigInt(position.tokensOwed1) > 0,
        uncollectedFees: {
          token0: position.tokensOwed0,
          token1: position.tokensOwed1,
          token0Formatted: (BigInt(position.tokensOwed0) / BigInt(10 ** 18)).toString(),
          token1Formatted: (BigInt(position.tokensOwed1) / BigInt(10 ** 18)).toString()
        }
      });
    } catch (error) {
